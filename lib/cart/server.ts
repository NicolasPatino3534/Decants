...
export async function buildCartLinesFromItems(
...
export function selectCartLinesForItems(
  items: CartItemInput[],
  lines: CartLine[],
) {
...
export async function getPersistedCartLines(
  supabase: SupabaseClient,
  userId: string,
  resolverSupabase = supabase,
) {
...
export async function replacePersistedCart(
  supabase: SupabaseClient,
  userId: string,
  lines: CartLine[],
  resolverSupabase = supabase,
  ...
  if (insertError)
    return { ok: false, error: insertError.message, lines: refreshedLines };
...
async function buildLegacyCartLines(
  supabase: SupabaseClient,
  items: CartItemInput[],
) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id,size_ml,price_cents,stock_on_hand,is_active,products ( id, name, slug, product_images ( storage_path, sort_order ) )",
    )
    .in(
      "id",
...
async function buildProductVariantCartLines(
  supabase: SupabaseClient,
  items: CartItemInput[],
) {
  const quantities = new Map(
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id,size_ml,price_cents,stock,active,products ( id, name, slug, product_images ( storage_path, public_url, sort_order, is_primary ) )",
    )
    .in(
      "id",
      items.map((item) => item.variantId),
  ...
  return (data as unknown as ProductVariantRow[])
    ...
    .map((variant) => ({
      ...
      imageUrl: resolveProductImageUrl(
        variant.products?.product_images,
        supabase,
      ),
      variantId: variant.id,
      sizeMl: Number(variant.size_ml),
...
function resolveLegacyImageUrl(
  images:
    NonNullable<LegacyVariantRow["products"]>["product_images"] | undefined,
) {
...
function resolveProductImageUrl(
  images:
    NonNullable<ProductVariantRow["products"]>["product_images"] | undefined,
  supabase: SupabaseClient,
) {
  const image = [...(images ?? [])].sort(
    (a, b) =>

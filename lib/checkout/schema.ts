...
const optionalText = (max: number) =>
  z.string().transform(sanitizePlainText).pipe(z.string().max(max)).optional().or(z.literal(""));
...
export const checkoutSchema = z.object({
  ...
  customer: z.object({
    ...
    email: z.string().transform((value) => sanitizePlainText(value).toLowerCase()).pipe(z.string().email("Ingresá un email válido.").max(160)),

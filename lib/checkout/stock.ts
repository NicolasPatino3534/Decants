...
export function selectCheckoutVariantsForItems<
  TVariant extends CheckoutStockVariant,
>(items: CheckoutItemInput[], variants: TVariant[]) {
...
export function buildCheckoutLines<TVariant extends CheckoutStockVariant>(
  ...
  return normalizedItems.map((item) => {
    ...
    if (variant.stockOnHand < item.quantity) {
      throw new CheckoutStockError(
        `${variant.productName} ${variant.sizeMl}ml no tiene stock suficiente.`,
      );
    }

        ...
        <section className="mt-7 rounded-md border border-line bg-paper p-5 shadow-soft">
          ...
          <div className="mt-4 divide-y divide-line">
            {order.items.map((item) => (
              <div key={item.id} className="flex justify-between gap-4 py-3 text-sm text-ink">
                <span>
                  {item.productName} <span className="text-soft">{item.variantLabel} x {item.quantity}</span>
                </span>

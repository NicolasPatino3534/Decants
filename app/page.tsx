    ...
    <main className="premium-shell">
        ...
        <div className="mx-auto grid min-h-[calc(100svh-64px)] max-w-7xl items-center gap-10 px-4 py-10 sm:px-6 lg:min-h-[690px] lg:grid-cols-[0.86fr_1.14fr] lg:px-8">
          <div className="max-w-2xl">
            <div className="mb-7 flex items-center gap-3">
              ...
              <p className="text-xs font-black uppercase tracking-[0.2em] text-[var(--accent-muted)]">{brand.tagline}</p>
            </div>
            ...
            <form action="/catalogo" className="mt-8 grid gap-3 rounded-md border border-line bg-paper p-2 shadow-soft sm:grid-cols-[1fr_auto]">
              ...
              <button className="inline-flex h-12 items-center justify-center gap-2 rounded-md bg-amber px-5 text-sm font-bold text-white transition hover:bg-[var(--accent-hover)]">
            ...
            <div className="mt-8 grid grid-cols-3 gap-3 border-y border-line py-5 text-sm text-muted">
              <p><span className="font-display block text-3xl text-ink">+40</span> perfumes curados</p>
              <p><span className="font-display block text-3xl text-ink">24h</span> preparación</p>
              <p><span className="font-display block text-3xl text-ink">3</span> tamaños para probar</p>
            </div>
          </div>
          ...
          <div className="relative min-h-[420px] overflow-hidden rounded-md border border-line bg-[var(--surface-strong)] shadow-[var(--shadow-lifted)] sm:min-h-[560px]">
            ...
            <div className="absolute inset-x-4 bottom-4 grid gap-3 rounded-md border border-line bg-paper/90 p-4 backdrop-blur sm:grid-cols-[1fr_auto] sm:items-center">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--accent-muted)]">Selección recomendada</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--accent-muted)]">Selección boutique</p>
            <h2 className="font-display mt-2 text-4xl tracking-normal text-ink sm:text-5xl">Decants para decidir mejor</h2>
            <p className="mt-3 max-w-2xl leading-7 text-muted">Elegí por familia olfativa, ocasión y tamaño. Cada tarjeta está pensada para comparar rápido sin perder detalle.</p>
          </div>
      ...
      <section id="discovery-sets" className="border-y border-line bg-paper py-16">
        <div className="mx-auto grid max-w-7xl gap-9 px-4 sm:px-6 lg:grid-cols-[0.85fr_1.15fr] lg:px-8">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--accent-muted)]">Packs de descubrimiento</p>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {packs.map((pack) => (
              <article key={pack.title} className="rounded-md border border-line bg-mist p-5">
                <div className="grid h-10 w-10 place-items-center rounded-md bg-amber text-white">
                  <Sparkles size={18} />
                </div>
                <h3 className="font-display mt-5 text-2xl text-ink">{pack.title}</h3>
                <p className="mt-3 text-sm leading-6 text-muted">{pack.text}</p>
                <p className="mt-4 border-t border-line pt-4 text-xs font-bold uppercase tracking-[0.12em] text-soft">{pack.notes}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section id="marcas" className="border-y border-line bg-paper py-16 text-ink">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--accent-muted)]">Marcas destacadas</p>
              <h2 className="font-display mt-2 text-4xl sm:text-5xl">Casas para explorar por perfil</h2>
            </div>
            <p className="max-w-md text-sm leading-6 text-muted">Una selección corta transmite curaduría. El catálogo completo mantiene filtros simples para profundizar.</p>
          </div>
          <div className="mt-9 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {(brands.length ? brands : ["Al Haramain", "Armaf", "Lattafa"]).map((brandName) => (
              <a key={brandName} href="/catalogo" className="flex items-center justify-between rounded-md border border-line bg-mist p-5 transition hover:border-[var(--border-strong)] hover:bg-warm">
      ...
      <section id="como-funciona" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.82fr_1.18fr]">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--accent-muted)]">Cómo funciona</p>
      ...
      <section className="border-y border-line bg-mist py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--accent-muted)]">Reviews verificadas</p>
              <h2 className="font-display mt-2 text-4xl text-ink sm:text-5xl">Confianza antes de confirmar</h2>
            </div>
            <div className="flex gap-1 text-amber" aria-label="5 estrellas">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {reviews.map((review) => (
              <article key={review.name} className="rounded-md border border-line bg-paper p-5">
      ...
      <section id="faq" className="mx-auto grid max-w-7xl gap-8 px-4 py-16 sm:px-6 lg:grid-cols-[0.75fr_1.25fr] lg:px-8">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--accent-muted)]">FAQ</p>
        ...
        <div className="divide-y divide-line rounded-md border border-line bg-paper">
          {faqs.map((faq) => (
            <details key={faq.q} className="group p-5">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">{faq.a}</p>
            </details>
          ))}
        </div>
      </section>
...
function Proof({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
    <div className="flex gap-3 rounded-md border border-line bg-paper p-4">
      <div className="grid h-10 w-10 shrink-0 place-items-center rounded-md bg-amber text-white">{icon}</div>
      <div>
...
function Step({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
    <div className="rounded-md border border-line bg-paper p-5">
      <div className="grid h-10 w-10 place-items-center rounded-md bg-warm text-[var(--accent-muted)]">{icon}</div>

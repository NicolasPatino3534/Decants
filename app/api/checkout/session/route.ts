...
export async function POST(request: Request) {
  ...
  const shippingMethod = await resolveShippingMethod(admin, parsed.data.shippingMethodId);
...
async function reserveStock(
  ...
  for (const line of lines) {
    ...
    reserved.push({ variant: line.variant, previousStock: line.variant.stockOnHand, nextStock });
  }
...
async function releaseStock(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, reserved: ReservedStock[]) {
  await Promise.all(
    reserved.map((item) =>
      admin
        .from(item.variant.table)
        .update({ [item.variant.stockColumn]: item.previousStock })
        .eq("id", item.variant.id)
        .eq(item.variant.stockColumn, item.nextStock),
    ),
  );
}

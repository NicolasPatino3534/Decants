    ...
    <div>
      ...
      <section className="mt-6 overflow-hidden rounded-md border border-line bg-white">
        <div className="grid gap-3 p-3 lg:hidden">
          {orders.map((order) => (
            <article key={order.id} className="rounded-md border border-line bg-[#fbfaf6] p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-black text-ink">#{order.orderNumber}</p>
                  <p className="mt-1 break-words text-sm font-bold text-ink">{order.customerName}</p>
                  <p className="mt-1 break-all text-xs text-[#756b5d]">{order.customerEmail}</p>
                </div>
                <p className="font-black text-ink">{formatMoney(order.totalCents)}</p>
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
                <StatusBadge tone={order.paymentStatus === "paid" ? "green" : "amber"}>{order.paymentStatus}</StatusBadge>
                <StatusBadge tone={order.status === "cancelled" ? "red" : order.status === "delivered" ? "green" : "neutral"}>
                  {order.status}
                </StatusBadge>
              </div>
              <p className="mt-3 text-sm text-[#665d50]">{new Date(order.createdAt).toLocaleString("es-AR")}</p>
              <Link
              ...
              {orders.map((order) => (
                <tr key={order.id} className="align-top">
                  ...
                  <td className="px-4 py-4">
                    ...
                    <p className="mt-1 text-xs text-[#756b5d]">{order.customerEmail}</p>
                  </td>
                  <td className="px-4 py-4 text-[#665d50]">{new Date(order.createdAt).toLocaleString("es-AR")}</td>
                  <td className="px-4 py-4 font-black text-ink">{formatMoney(order.totalCents)}</td>
                  <td className="px-4 py-4">
                    <StatusBadge tone={order.paymentStatus === "paid" ? "green" : "amber"}>{order.paymentStatus}</StatusBadge>
                  </td>
                  <td className="px-4 py-4">
        ...
        {orders.length === 0 ? <p className="p-6 text-sm text-[#665d50]">Todavia no hay pedidos.</p> : null}
      </section>
    </div>
  );
}

    ...
    <div>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-neutral-600">Gestión de catálogo, imágenes, variantes y perfil olfativo.</p>
        </div>
      </div>

      <section className="mt-5 rounded-md border border-line bg-white p-4">
        <h2 className="text-base font-black">Nuevo producto</h2>
        <form action={createProduct} className="mt-4 grid gap-3 text-sm md:grid-cols-4" encType="multipart/form-data">
          ...
          <Select name="categoryId" label="Categoría" options={categories.map((category) => ({ label: category.name, value: category.id }))} />
          <Input name="concentration" label="Concentración" defaultValue="Eau de Parfum" />
          <Select name="gender" label="Género" options={genders.map((value) => ({ label: value, value }))} />
          <Select name="status" label="Estado" options={statuses.map((value) => ({ label: value, value }))} />
          <Input name="durationEstimate" label="Duración estimada" placeholder="6 a 8 horas" />
          <Input name="projectionEstimate" label="Proyección" placeholder="Media alta" />
          <Input name="recommendedOccasion" label="Ocasión" placeholder="Noche, eventos" />
          <Input name="recommendedSeason" label="Estación" placeholder="Otoño / invierno" />
          <Input name="notesTop" label="Notas de salida" placeholder="Bergamota, lima" />
          <Input name="notesHeart" label="Notas de corazón" placeholder="Cedro, neroli" />
          <Input name="notesBase" label="Notas de fondo" placeholder="Ámbar, musk" />
          <label className="md:col-span-3">
            <span className="mb-1 block text-xs font-bold">Descripción</span>
          ...
          <label>
            <span className="mb-1 block text-xs font-bold">Imagen</span>
          ...
          <Button className="md:col-span-4 md:w-fit">Crear producto</Button>
        </form>
      </section>
...
function ProductEditor({
  ...
  return (
    <article className="rounded-md border border-line bg-white p-4">
      ...
      <form action={updateProduct} className="mt-4 grid gap-3 text-sm md:grid-cols-4" encType="multipart/form-data">
        <input type="hidden" name="id" value={product.id} />
        ...
        <Select name="categoryId" label="Categoría" defaultValue={product.category.id} options={categories.map((category) => ({ label: category.name, value: category.id }))} />
        ...
        <label>
          <span className="mb-1 block text-xs font-bold">Nueva imagen</span>
          <input name="image" type="file" accept="image/*" className="h-10 w-full rounded-md border border-line px-3 py-2 text-sm" />
        </label>
        <label className="flex items-center gap-2 text-sm font-semibold md:col-span-4">
      ...
      <div className="mt-4 rounded-md border border-line">
        <div className="border-b border-line bg-mist px-4 py-2 text-sm font-black">Variantes</div>
        <div className="divide-y divide-line">
          {product.variants.map((variant) => (
            <form key={variant.id} action={upsertVariant} className="grid gap-3 p-3 md:grid-cols-[0.8fr_1fr_1fr_0.8fr_0.8fr_auto_auto]">
          ...
          <form action={upsertVariant} className="grid gap-3 p-3 md:grid-cols-[0.8fr_1fr_1fr_0.8fr_0.8fr_auto]">
            <input type="hidden" name="productId" value={product.id} />
  ...
  return (
    <label>
      ...
      <input
        ...
        required={required}
        className="h-10 w-full rounded-md border border-line px-3 text-sm outline-none focus:border-ink"
      />
    </label>
  );
}

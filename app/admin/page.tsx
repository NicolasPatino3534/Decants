...
export default async function AdminBalancePage() {
  ...
  const averageTicket =
    paidOrderCount > 0 ? Math.round(revenue / paidOrderCount) : 0;
  const now = new Date();
    ...
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[#665d50]">
            Resumen básico para revisar ventas sin sumar secciones fuera del
            alcance del panel temporal.
          </p>
        </div>
      </div>
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Kpi
          ...
          helper="Pedidos con pago confirmado"
        />
        <Kpi
          icon={<ClipboardList size={19} />}
          label="Cantidad de pedidos"
          value={String(orderCount)}
          helper={`${paidOrderCount} pagos confirmados`}
        />
        <Kpi
          icon={<WalletCards size={19} />}
          label="Ticket promedio"
          value={formatMoney(averageTicket)}
          helper="Sobre pedidos pagados"
        />
        <Kpi
          icon={<ReceiptText size={19} />}
          label="Ingresos del mes"
          value={formatMoney(currentMonthRevenue)}
          helper="Periodo calendario actual"
        />
      </section>
        ...
        <div className="rounded-md border border-line bg-white p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <Summary
              ...
              value={formatMoney(lastSevenDaysRevenue)}
            />
            <Summary
        ...
        <div className="rounded-md border border-line bg-white p-5">
          ...
          <div className="mt-4 grid gap-3">
            <AdminLink
              ...
              title="Pedidos"
              text="Ver pedidos y entrar al detalle."
            />
            <AdminLink
              href="/admin/catalogo"
              icon={<PackageSearch size={18} />}
              title="Catálogo"
              text="Ver productos y editar datos."
            />
          </div>
        </div>
      </section>
...
function Kpi({
  ...
  return (
    <div className="rounded-md border border-line bg-white p-5">
      <div className="grid h-10 w-10 place-items-center rounded-md bg-[#f6edda] text-[#8a611c]">
        {icon}
      </div>
...
function Summary({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-line bg-[#fbfaf6] p-4">
      <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#756b5d]">
        {label}
      </p>
...
function AdminLink({
  ...
  return (
    <Link
      href={href}
      className="flex items-center justify-between gap-4 rounded-md border border-line p-4 transition hover:border-[#caa55c] hover:bg-[#fbf7ed]"
    >
      <span className="flex items-center gap-3">
        <span className="grid h-10 w-10 place-items-center rounded-md bg-[#b8872f] text-white">
          {icon}
        </span>
        <span>

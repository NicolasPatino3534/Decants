    ...
    return (
      <main className="premium-shell mx-auto grid min-h-[70vh] place-items-center px-4 py-16 text-center">
        <div className="max-w-md rounded-md border border-line bg-white p-8 shadow-[0_18px_50px_rgba(11,13,15,0.08)]">
          ...
          <h1 className="font-display mt-5 text-4xl text-ink">No hay items para pagar</h1>
          <p className="mt-3 text-sm text-[#6f6658]">Agrega un decant al carrito para continuar.</p>
  ...
  return (
    <main className="premium-shell">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_400px] lg:px-8 lg:py-12">
        <section>
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#8c682b]">Checkout seguro</p>
          ...
          <ol className="mt-5 grid gap-2 text-sm font-bold text-[#5f574c] sm:grid-cols-3">
            ...
            <Step icon={<MapPin size={16} />} label="Datos de envio" active />
            <Step icon={<CreditCard size={16} />} label="Pago protegido" />
          </ol>
          <form id="checkout-form" action={startCheckout} className="mt-6 grid gap-5 rounded-md border border-line bg-white p-5 shadow-[0_18px_50px_rgba(11,13,15,0.06)]">
            ...
            <section>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
              <Field name="postalCode" label="Codigo postal" autoComplete="postal-code" required />
              <Field name="street" label="Direccion" autoComplete="street-address" className="md:col-span-2" required />
            ...
            <section>
              <h2 className="flex items-center gap-2 text-lg font-black text-ink">
                <PackageCheck size={18} className="text-[#5f7d69]" /> Metodo y cupon
              </h2>
              <div className="mt-4 grid gap-4">
                <label>
                  <span className="mb-1 block text-xs font-bold uppercase tracking-[0.12em] text-[#7b7164]">Metodo de envio</span>
                  <select
                  ...
                  <span className="mt-1 block text-xs text-[#7d7467]">
                    {selectedShipping.description ?? "Envio con seguimiento."}
                  </span>
                </label>

                <label>
                  <span className="mb-1 block text-xs font-bold uppercase tracking-[0.12em] text-[#7b7164]">Cupon de descuento</span>
                  <input
                    value={couponCode}
            ...
            <div className="flex items-center gap-2 rounded-md bg-[#f5faf6] p-3 text-sm font-semibold text-[#47624f]">
              <Truck size={17} />
              El envio se confirma con tracking cuando el pedido pasa a preparacion.
            </div>
            {error ? (
        ...
        <aside className="h-fit rounded-md border border-line bg-white p-5 shadow-[0_18px_50px_rgba(11,13,15,0.08)] lg:sticky lg:top-28">
          ...
          <div className="mt-5 grid gap-2 text-xs font-semibold text-[#5f665d]">
            <span className="flex items-center gap-2 rounded-md bg-mist p-3"><CheckCircle2 size={15} /> Costos visibles antes de pagar</span>
            <span className="flex items-center gap-2 rounded-md bg-mist p-3"><Lock size={15} /> Redireccion a pago seguro</span>
          </div>

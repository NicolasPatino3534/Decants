...
export function AddToCartPanel({ product }: { product: Product }) {
  ...
  const quantity = Math.min(
    ...
    Math.max(variant.stockOnHand, 1),
  );
  const stockLabel =
  ...
  return (
    <div
      className={`rounded-md border border-line bg-paper p-5 shadow-soft transition-[border-color,box-shadow] duration-[var(--motion-base)] ease-[var(--ease-premium)] ${added ? "motion-feedback border-amber shadow-[0_24px_70px_rgba(184,135,47,0.18)]" : ""}`}
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-muted">
            Atomizador rotulado, preparado con stock real y listo para probar.
          </p>
        </div>
          ...
          {product.variants.map((item) => {
            ...
            return (
              <button
                ...
                className={`min-h-[92px] rounded-md border px-3 py-3 text-left text-sm transition-[transform,background-color,border-color,box-shadow] duration-[var(--motion-fast)] ease-[var(--ease-premium)] active:scale-[0.99] ${
                  isSelected
                    ? "border-amber bg-amber text-[var(--on-accent)]"
              ...
              >
                ...
                <span className="mt-2 block text-[11px] font-bold uppercase tracking-[0.08em]">
                  {getSizeUse(item.sizeMl)}
                </span>
              </button>
            );
          })}
      ...
      <div className="mt-5 flex flex-col gap-3 rounded-md border border-line bg-mist p-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex h-11 w-fit items-center rounded-md border border-line bg-paper">
          <button
            type="button"
            className="grid h-11 w-11 place-items-center rounded-md text-ink transition-[background-color,transform] duration-[var(--motion-fast)] ease-[var(--ease-premium)] hover:bg-warm active:scale-95 disabled:opacity-35"
            disabled={quantity <= 1}
            onClick={() =>
          ...
          <span
            className="grid h-11 w-10 place-items-center text-sm font-bold text-ink"
            aria-live="polite"
          >
            {quantity}
          </span>
          <button
            ...
            onClick={() =>
              setRequestedQuantity((current) =>
                Math.min(variant.stockOnHand, current + 1),
              )
            }
      ...
      <div className="mt-4 grid gap-2 text-xs font-semibold text-muted sm:grid-cols-3">
        <span className="flex items-center gap-1.5 rounded-md bg-mist p-2">
          <ShieldCheck size={14} /> Compra segura
        </span>
        <span className="flex items-center gap-1.5 rounded-md bg-mist p-2">
          <CreditCard size={14} /> Confirmación clara
        </span>
        <span className="flex items-center gap-1.5 rounded-md bg-mist p-2">
          <BadgeCheck size={14} /> Stock real
        </span>
      </div>
    </div>
  );

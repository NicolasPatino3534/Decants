  ...
  return (
    <header className="sticky top-0 z-40 border-b border-line bg-paper/95 backdrop-blur-xl">
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:h-[76px] lg:px-8">
        <Link href="/" className="flex min-w-0 items-center gap-3 text-ink" onClick={() => setOpen(false)}>
          ...
          <span className="min-w-0">
            <span className="font-display block truncate text-xl font-semibold leading-none tracking-normal">{brand.displayName}</span>
            <span className="mt-1 hidden text-[11px] font-bold uppercase tracking-[0.18em] text-soft sm:block">Perfumes originales</span>
          </span>
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <div ref={accountRef} className="relative hidden lg:inline-flex">
            <button
              type="button"
              className="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-line bg-paper px-3 text-sm font-bold text-ink transition hover:border-[var(--border-strong)] hover:bg-warm focus:outline-none focus:ring-2 focus:ring-amber focus:ring-offset-2 focus:ring-offset-bg"
            ...
            {accountOpen ? (
              <div className="absolute right-0 top-12 w-72 rounded-md border border-line bg-paper p-2 shadow-[var(--shadow-lifted)]">
                {user ? (
                  <>
                    <div className="px-3 py-2">
                      <p className="text-sm font-black text-ink">{displayName}</p>
                      {accountEmail ? <p className="mt-1 truncate text-xs font-semibold text-soft">{accountEmail}</p> : null}
                    </div>
                  ...
                  <>
                    <p className="px-3 py-2 text-xs font-bold uppercase tracking-[0.14em] text-soft">Cuenta</p>
      ...
      {loginGreeting ? (
        <div className="border-t border-line bg-warm">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 text-sm font-bold text-[var(--accent-muted)] sm:px-6 lg:px-8">
            ...
            <button type="button" className="grid h-7 w-7 place-items-center rounded-md hover:bg-paper" aria-label="Cerrar saludo" onClick={() => setLoginGreeting(null)}>
              <X size={15} />
            </button>
          </div>
        </div>
      ...
      {open ? (
        <div className="border-t border-line bg-paper px-4 pb-4 shadow-[var(--shadow-lifted)] lg:hidden">
          <div className="flex items-center justify-between border-b border-line py-3 sm:hidden">
            <span className="text-xs font-bold uppercase tracking-[0.14em] text-soft">Tema</span>
            <ThemeToggle />
          </div>
          ...
          <div className="border-t border-line py-3">
            {user ? (
              <div className="grid gap-1 text-sm font-bold text-ink">
                <p className="px-3 py-2 text-xs font-black uppercase tracking-[0.14em] text-soft">Hola, {displayName}</p>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs font-semibold text-[var(--accent-muted)]">
            ...
            <span className="rounded-md bg-warm p-3">Tracking en envíos</span>
          </div>
        </div>
      ) : null}
    </header>

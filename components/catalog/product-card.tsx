...
export function ProductCard({
  ...
  const lowStock = availableVariants.some(
    (variant) => variant.stockOnHand <= variant.lowStockThreshold,
  );
  const topNotes = [...product.notesTop, ...product.notesHeart].slice(0, 3);

  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-md border border-line bg-paper transition-[transform,border-color,box-shadow] duration-[var(--motion-base)] ease-[var(--ease-premium)] hover:-translate-y-1 hover:border-[var(--border-strong)] hover:shadow-soft">
      <Link
        href={`/producto/${product.slug}`}
        className="relative block aspect-[4/3] overflow-hidden bg-[var(--surface-strong)]"
      >
        <Image
          ...
          className="object-contain object-center transition-transform duration-[var(--motion-slow)] ease-[var(--ease-premium)] group-hover:scale-[1.03]"
        />
        <span className="absolute left-3 top-3 rounded-md bg-amber px-3 py-1 text-xs font-bold text-[var(--on-accent)] backdrop-blur">
          {isSoldOut
            ? "Agotado"
            : lowStock
              ? "Últimas unidades"
              : `${firstVariant?.sizeMl ?? 2}ml disponible`}
        </span>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 sm:p-5">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-soft">
            {product.brand.name}
          </p>
          <Link
            href={`/producto/${product.slug}`}
            className="font-display mt-1 block text-2xl leading-tight text-ink hover:underline"
          >
            {product.name}
          </Link>
        ...
        <div className="flex min-h-[76px] flex-wrap content-start gap-2 text-xs font-bold">
          ...
          {topNotes.map((note) => (
            <span
              key={note}
              className="rounded-md border border-line px-2.5 py-1.5 text-muted"
            >
              {note}
            </span>
          ))}
        ...
        <div className="mt-auto grid gap-3 border-t border-line pt-4">
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              {firstVariant &&
              lastVariant &&
              firstVariant.id !== lastVariant.id ? (
                <p className="text-xs text-soft">
                  Hasta {formatMoney(lastVariant.priceCents)}
                </p>
              ) : null}
            </div>

...
describe("Supabase production hardening SQL", () => {
  ...
  it("keeps admin write policies behind an admin role helper", () => {
    ...
    expect(hardeningSql).toContain("public.custom_access_token_hook(event jsonb)");
    expect(hardeningSql).not.toContain("with check (true)");
  });

  it("does not query profiles from the complete-schema admin helper", () => {
    const isAdminFunction = completeSql.match(/create or replace function public\.is_admin\(\)[\s\S]*?\$\$;/)?.[0] ?? "";
  ...
  it("defines atomic stock reservation and release functions", () => {
    ...
    expect(combinedSql).toContain("public.reserve_checkout_stock(p_items jsonb)");
    expect(combinedSql).toContain("public.increment_variant_stock(p_variant_id uuid, p_quantity integer)");
    expect(combinedSql).toContain("public.increment_decant_variant_stock(p_variant_id uuid, p_quantity integer)");
  });
});

...
describe("mercadopago webhook helpers", () => {
  it("extracts payment ids from current webhook query params", () => {
    const url = new URL(
      "https://example.com/api/webhooks/mercadopago?type=payment&data.id=12345",
    );
  ...
  it("extracts payment ids from legacy query and resource formats", () => {
    expect(
      getMercadoPagoPaymentId(
        {},
        new URL(
          "https://example.com/api/webhooks/mercadopago?topic=payment&id=987",
        ),
      ),
    ).toBe("987");
    expect(
      getMercadoPagoPaymentId(
        {
          resource: "https://api.mercadopago.com/v1/payments/654321",
          topic: "payment",
        },
  ...
  it("extracts order ids and maps payment statuses", () => {
    ...
    expect(getMercadoPagoOrderId({ external_reference: "order-2" })).toBe(
      "order-2",
    );
    ...
    expect(getMercadoPagoPaymentAction("refunded")).toBe("reversed");
    expect(getMercadoPagoPaymentAction("charged_back")).toBe("reversed");
